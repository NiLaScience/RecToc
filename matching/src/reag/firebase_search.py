...
from .utils import format_job_content
...
class FirebaseVectorSearch:
    ...
    def _convert_to_document(self, doc_snapshot) -> Document:
        ...
        metadata = doc_dict.get(self.config.metadata_field, {})
        
        # Log document conversion
        print(f"[DEBUG] Converting document {doc_snapshot.id}")
        print(f"[DEBUG] Original metadata: {metadata}")
        
        # Ensure job ID is in metadata
        metadata['jobId'] = doc_snapshot.id
        
        print(f"[DEBUG] Updated metadata with jobId: {metadata}")
        
        return Document(
            name=doc_snapshot.id,
            content=format_job_content(doc_dict) if self.config.content_field == "" else doc_snapshot.get(self.config.content_field),
            metadata=metadata
        )
    ...
    async def load_documents(
        ...
        try:
            ...
            docs = query.get()
            print(f"[DEBUG] Loaded {len(docs)} documents from Firestore")
            
            # Log first document for debugging
            if docs:
                first_doc = docs[0]
                print(f"[DEBUG] First document ID: {first_doc.id}")
                print(f"[DEBUG] First document data: {first_doc.to_dict()}")
            
            return [self._convert_to_document(doc) for doc in docs]
