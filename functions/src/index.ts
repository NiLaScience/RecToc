...
async function generateSlidesContent(jobData: JobDescription, jobId: string): Promise<{ slides: SlideData[] }> {
  ...
  try {
    const styles = [
      ...
      "minimalist digital art style",
      "modern flat design illustration",
      "vibrant geometric art",
      "clean vector art style",
      "trendy tech aesthetic",
      "playful 3D render",
      "isometric design style",
      "pop art inspired",
      "neon-infused tech art",
      "material design inspired"
    ];
    const chosenStyle = styles[Math.floor(Math.random() * styles.length)];
    console.log('Selected style:', chosenStyle);

    const promptMessage = `You are an expert in creating marketing slide content for modern mobile videos, like TikToks.
    
Generate a JSON array of exactly 4 slides for a job posting. Return ONLY the raw JSON array without any markdown formatting, code blocks, or backticks.
...
4-6 bullet points per slide. max 6 words each.

The 4 slides should be:
1. Title: "2-3 word tagline for the job and company", heading: "max 6 words summary of company, job title, and location", bullets: "details on job title, company, location, and best perk"
2. Title: "${jobData.company || "Name of Company"}" - heading: "max 5 words summary of company activities/brand/mission", bullets: "details on company activities/brand/mission"
3. Title "${jobData.title || "Job Title"}" - heading: "max 5 words summary of job impact", bullets: "key requirements, qualifications, and responsibilities"
4. Title: "Why Apply?" - heading: "max 5 words summary of uniqueness of the opportunity", bullets: "Top 4 reasons to apply"  Do not use generic phrases like 'career growth' or 'exciting projects'. Make it personal and specific to the job and company.

Make bullet points concise (max 6 words each).
Use known abbreviations where necessary (Sen, Dir. SVP, etc., Microsoft instead of Microsoft Corporation, AWS instead of Amazon Web Services, etc.)
Ensure the content is impactful, specific to the job and company, minimalistic, and optimized for TikTok-style presentation. Our goal is to stand out from the crowd and make it look human, not AI-generated.
The image prompts should be fun, interesting, and engaging for a younger, tech-savvy audience and will be enhanced with the style: ${chosenStyle} and then generated by DALL-E AI image generation model. Make them specific to the job, company and location.
...
Remember: Return ONLY the raw JSON array. No markdown. No \`\`\`. No explanation.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: "system", 
          content: "You generate structured slide content as JSON for TikTok-style job ads. Output only valid JSON arrays without any markdown formatting or explanation. Rule Number 1: don't be generic, make it specific to the job and company. Rule Number 2: don't use generic phrases like 'career growth' or 'exciting projects'. Make it personal and specific to the job and company."
        },
...
async function generateVoiceoverScript(jobData: JobDescription): Promise<string> {
  const prompt = `Create a word-for-word script for a 1-minute pitch about this job opportunity (120 words) for social media like TikTok.
  ...
  The script will be read by a voiceover model, so you can use capitalizations and punctuation to emphasize key points.
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are recruiter representing the hiring company writing scripts for job pitches that will be converted to audio using a tts model. Refer to the company as we/us. Your goal is to motive candidates to apply for the job. Do not waste any time on introductions or flowery language, you want to engage people as quickly as possible by getting them excited about what makes the job stand out. Don't use introductions like imagine or have you ever thought about or anything like that. Just get to the point and win people over for your company."
      },
...
async function generateVoiceoverAudio(script: string, jobId: string): Promise<string> {
  ...
  try {
    ...
    // Upload to Firebase Storage
    const bucket = admin.storage().bucket();
    const audioFileName = `voiceovers/${jobId}.mp3`;
...
${jobData.raw_content}`
      ...
      // Generate a unique ID for the job opening with prefix for crawled jobs
      const jobOpeningId = `crawled-${uuidv4()}`;
...
export const onJobOpeningCreate = functions.firestore
  .document('job_openings/{docId}')
  .onCreate(async (snapshot, context) => {
    const jobData = snapshot.data();
    console.log('Processing new job:', context.params.docId);

    // Skip if this is a crawled job (handled by onCrawledJobCreate)
    if (jobData.is_crawled) {
      console.log('Skipping crawled job');
      return;
    }

    try {
      // Generate slides
      const slidesResult = await generateSlidesContent(jobData.jobDescription, jobData.id);
      console.log('Generated slides result:', JSON.stringify(slidesResult, null, 2));
      
      // Update the job document with slides and set status to processing_voiceover
      await snapshot.ref.update({
        slides: slidesResult.slides,
        status: 'processing_voiceover',
        lastUpdated: admin.firestore.FieldValue.serverTimestamp()
      });

      console.log(`Successfully generated slides for job ${context.params.docId}`);
    } catch (error) {
      console.error(`Error processing job ${context.params.docId}:`, error);
...
export const onJobProcessingVoiceover = functions.firestore
  .document('job_openings/{docId}')
  .onUpdate(async (change, context) => {
    const afterData = change.after.data();
    const beforeData = change.before.data();

    // Only process if status changed to processing_voiceover
    if (afterData.status !== 'processing_voiceover' || beforeData.status === 'processing_voiceover') {
      return;
    }

    try {
      console.log('Generating voiceover for job:', context.params.docId);
      ...
      console.log(`Successfully processed voiceover for job ${context.params.docId}`);
    } catch (error) {
      console.error(`Error processing voiceover for job ${context.params.docId}:`, error);
